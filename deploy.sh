...
# Deploys App Service, Azure SQL, and application code (without GenAI services)
# 
# Prerequisites:
...
echo "Step 3: Deploying infrastructure (App Service, Managed Identity, SQL Database)..."
DEPLOYMENT_OUTPUT=$(az deployment group create \
    --resource-group $RESOURCE_GROUP \
    --template-file infrastructure/main.bicep \
    --parameters baseName=$BASE_NAME adminLogin="$ADMIN_LOGIN" adminObjectId=$ADMIN_OBJECT_ID deployGenAI=false \
...
echo "Step 11: Configuring App Service settings..."
...
az webapp config appsettings set \
    --name $WEB_APP_NAME \
    --resource-group $RESOURCE_GROUP \
    --settings "ConnectionStrings__DefaultConnection=$CONNECTION_STRING" "ManagedIdentityClientId=$MANAGED_IDENTITY_CLIENT_ID" \
    --output none
